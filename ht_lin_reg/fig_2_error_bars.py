...
class ToySGD:
    def __init__(self, dim: int, wandb_init: bool = False, wandb_config: dict = None, sigma: float = 1.) -> None:
        self.sigma_x = 3.
        self.sigma_y = 3.
        self.sigma_a = sigma
        ...
        if wandb_init:
            wandb.init(project="heavy_tails6", entity="klehman", config=wandb_config, dir="/home/klehmanp/scratch/wandb")
    ...
    def train(self, key: jax.random.PRNGKey, n_iter: int = 1e6, eta: float = None, b: int = None):
        ...
        for n in [10, 20, 30, 50, 100, 200, 300, 400, 500, 750, 1000]:
            key, subkey = jax.random.split(key)
            ...
            if self.wandb_log:
                wandb.log({"eta": eta, "alpha_estim": alpha_estim})
...
if __name__ == '__main__':  #
    ...
    est = ToySGD(dim=d, wandb_init=False)
    est.train(main_key, n_iter=n_iter_, eta=1e-2, b=batch_size)
